...
def noise_estimation_loss(model, x0, t, e, b, conditional = True):
...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        self.model = DiffusionUNet(config, config.data.conditional)
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                loss = noise_estimation_loss(self.model, x, t, e, b, self.config.data.conditional)

                self.tb_logger.add_scalar("loss", loss, global_step=self.step)
    ...
    def sample_image(self, x_cond, x, eta=0., last=True, patch_locs=None, patch_size=None):
        ...
        if patch_locs is not None:
            ...
            xs = utils.sampling.generalized_steps(x, x_cond, seq, self.model, self.betas, eta=eta)
        if last:
            xs = xs[0][-1]
        return xs

    def sample_image_unconditional(self, x, eta, last=True):
        skip = self.config.diffusion.num_diffusion_timesteps // self.args.sampling_timesteps
        seq = range(0, self.config.diffusion.num_diffusion_timesteps, skip)
        
        xs = utils.sampling.generalized_steps_unconditional(x, seq, self.model, self.betas, eta=eta)
