...
class DiffusiveRestoration:
    ...
    def restore_(self, img_path, r=None):
        # image_folder = os.path.join(self.args.image_folder, self.config.data.dataset, validation)
        ...
        with torch.no_grad():
            ...
            # x = x.flatten(start_dim=0, end_dim=1) if x.ndim == 5 else x
            # x_cond = x[:, :3, :, :].to(self.diffusion.device)
            x_cond = Image.open(img_path)
            x_cond = x_cond.resize((256,256), Image.Resampling.LANCZOS)
            x_cond = to_tensor(x_cond).to(self.diffusion.device)
            utils.logging.save_image(x_cond, f"results/input.png")
            x_cond = x_cond[None, :, :, :]
            x_output = self.diffusive_restoration(x_cond, r=r)
            x_output = inverse_data_transform(x_output)
            utils.logging.save_image(x_output, f"results/seq5/output.png")
